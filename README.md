...
### Description ###
Upload and manage files with autoForm. Since yogiben:autoform-file is outdated and do not work
...
2) Install `meteor add cfs:filesystem`

3) Install `meteor add cfs:standard-packages`

4) Install `meteor add meanware:gridfs`

5) Create your collectionFS (See [collectionFS](https://github.com/CollectionFS/Meteor-CollectionFS))
```coffeescript
@Images = new FS.Collection("images",
  stores: [new FS.Store.GridFS("images", {})]
)
```
6) Make sure the correct allow rules & subscriptions are set up on the collectionFS
```coffeescript
Images.allow
  insert: (userId, doc) ->
    true
...
```
7) Define your schema and set the `autoform` property like in the example below
```coffeescript
Schemas = {}
...
8) Generate the form with `{{> quickform}}` or `{{#autoform}}`
